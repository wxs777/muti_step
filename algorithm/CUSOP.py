from single_step.AutoSynRoute.inference import Autosyn_Inference
...
class ContinuousUnconstrainedSingleOpt(pymooProblem):
    ...
    def evaluate_indivisual(self, iter_num, indis):
       ...
       # tgt_smi = 'CN1CCN(c2ccc(-c3cccc(C(Br)N4CCOCC4)c3)cc2NC(=O)c2cnc(F)cc2C(F)(F)F)CC1'
        root_path = r'C:\Users\www11\Downloads\EvoRRP-main\single_step\AutoSynRoute'
        beam_size = 10
        pretrained_path = r'C:\Users\www11\Downloads\EvoRRP-main\algorithm\Models/AutoSynRoute/checkpoints/USPTO_50K/model.pt'
 #       top_k, score = Autosyn_Inference(root_path, pretrained_path, beam_size).api_model_pred_reactant(tgt_smi)

        #reactant_predictor = Autosyn_Inference(root_path, pretrained_path, beam_size).api_model_pred_reactant(tgt_smi)  # 用于单步预测的类 ， 替换成自己的单步模型
        ...
        for j, indi in enumerate(indis):  # 遍历个体的每个节点 ，indi为0-1之间的值，需要转换成react
            ...
            if j == 0:
                smi_nodes_sorted, prob_nodes_sorted = Autosyn_Inference(root_path, pretrained_path, beam_size).api_model_pred_reactant(self.config['target_smi']) # 替换成自己的单步模型
                ...
                smi_nodes_sorted, prob_nodes_sorted = Autosyn_Inference(root_path, pretrained_path, beam_size).api_model_pred_reactant(next_node)  # 替换成自己的单步模型
                if len(smi_nodes_sorted) <= init_node:
                    logger.debug('next_node: {}'.format(next_node))
                    logger.debug('smi_nodes_sorted: {}'.format(smi_nodes_sorted))
                    logger.debug('len(smi_nodes_sorted): {}  init_node: {}'.format(len(smi_nodes_sorted), init_node))
    ...
    def _f_x(self, score_cls_list, eval_method):
        ...
        for score_cls in score_cls_list:
            # 检查 score_cls 是否为 numpy.float64 类型
            if isinstance(score_cls, np.float64):
                observe.append(score_cls)  # 直接加入 observe 列表
            else:
                observe.append(score_cls.get())  # 如果是其他类型，调用 .get()
